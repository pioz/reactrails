...
1. Add `import 'reactrails'` in your `application.js`.
...
6. Creates a symbolic link inside `node_modules/reactrails` that points to the gem's JavaScript source directory.
...
2. Let reactrails js find that node, instantiate the React component, and mount it via `createRoot`.
...
3. On the client side, reactrails js hydrates the existing markup using `hydrateRoot`.
...
- It reads:
  ...
  - The gem's server-side helper bundle: `ssr.js`
...
This runs the scripts defined in `package.json` and regenerates the files
under `vendor/` dir of the gem.
